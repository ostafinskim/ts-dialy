...
type GenerateSignatureArgs = {
	name: string;
	title: string;
	company: string;
}

export default function generateSignature({ name, title, company }: GenerateSignatureArgs): string {
	let firstLetter = name.charAt(0).toUpperCase()

	let prefix: string
	if (/^[A-I]$/.test(firstLetter)) {
		prefix = '>>'
	} else if (/^[J-R]$/.test(firstLetter)) {
		prefix = '--'
